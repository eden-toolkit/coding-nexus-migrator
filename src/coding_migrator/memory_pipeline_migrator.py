...
class MemoryMigrationTask:
    ...
    created_time: float = field(default_factory=time.time)  # 任务创建时间
...
class MemoryPipelineMigrator:
    ...
    def __init__(self, config: MigrationConfig):
        ...
        # POM 文件追踪和调试
        self.pom_stats = {
            'discovered_in_coding': 0,      # 在CODING中发现的POM文件数
            'skipped_already_uploaded': 0,  # 跳过已上传的POM文件数
            'download_attempted': 0,        # 尝试下载的POM文件数
            'download_success': 0,          # 下载成功的POM文件数
            'download_failed': 0,           # 下载失败的POM文件数
            'upload_attempted': 0,          # 尝试上传的POM文件数
            'upload_success': 0,            # 上传成功的POM文件数
            'upload_failed': 0,             # 上传失败的POM文件数
            'missing_in_nexus': []          # 最终缺失的POM文件列表
        }
        ...
        try:
            ...
            if not self.failed_download_log.exists():
                with open(self.failed_download_log, 'w', encoding='utf-8') as f:
                    f.write("# CODING Maven 制品下载失败记录\n")
                    f.write("# 格式: 时间戳 | Maven坐标 | 文件路径 | 错误信息\n")
            ...
            if not self.failed_upload_log.exists():
                with open(self.failed_upload_log, 'w', encoding='utf-8') as f:
                    f.write("# Nexus Maven 制品上传失败记录\n")
                    f.write("# 格式: 时间戳 | Maven坐标 | Maven路径 | 错误信息\n")
                    f.write("# Generated by Memory Pipeline Migrator\n\n")
    ...
    def _emergency_memory_cleanup(self) -> None:
        ...
        try:
            with self.memory_lock:
                # 清理失败的任务内存
                cleared_memory = 0
                for task in self.failed_tasks[:]:  # 复制列表，避免修改时出错
                    if hasattr(task, 'file_data') and task.file_data is not None:
        ...
        try:
            ...
            while not self.upload_queue.empty():
                try:
                    ...
                    # 检查任务是否等待时间过长
                    if hasattr(task, 'file_data') and task.file_data is not None:
                        # 如果任务创建时间超过5分钟，清理内存但保留任务
                        if hasattr(task, 'created_time'):
                            if current_time - task.created_time > 300:  # 5分钟
                                file_size = len(task.file_data)
                                self.current_memory_usage -= file_size
                                cleared_memory += file_size
                                task.file_data = None
    ...
    def _generate_final_stats(self) -> None:
        ...
        if self.failed_tasks:
            ...
            if pom_failed:
                ...
                for task in pom_failed[:3]:  # 显示前3个POM错误
        ...
        if missing_poms:
            logger.warning(f"⚠️  Found {len(missing_poms)} JAR files without corresponding POM files:")
            for missing in missing_poms[:5]:  # 显示前5个缺失的POM
    ...
    def _display_pom_detailed_report(self) -> None:
        """显示POM文件详细统计报告"""
        logger.info("")
        logger.info("📋 POM FILE DETAILED REPORT")
        logger.info("=" * 80)
        ...
        # 分析问题
        logger.info("")
        logger.info("🔍 POM FILE ANALYSIS:")

        if stats['discovered_in_coding'] == 0:
            logger.warning("⚠️  No POM files discovered in CODING repositories")
            ...
            if skipped_count > 0:
                logger.warning(f"⚠️  {skipped_count} POM files discovered but not attempted for download")
                logger.warning("   This could indicate:")
        ...
        if stats['download_failed'] > 0:
            ...
            logger.warning(f"⚠️  {stats['download_failed']} POM files failed to download ({failure_rate:.1f}% failure rate)")
        ...
        if stats['upload_failed'] > 0:
            ...
            logger.warning(f"⚠️  {stats['upload_failed']} POM files failed to upload ({failure_rate:.1f}% failure rate)")
            logger.warning("   Possible causes:")
        ...
        if stats['missing_in_nexus']:
            logger.warning(f"⚠️  {len(stats['missing_in_nexus'])} JAR files in Nexus are missing corresponding POM files:")
            for missing in stats['missing_in_nexus'][:10]:  # 显示前10个
                logger.warning(f"  - {missing}")
